...
def make_param_dict(peaks, data, lineshape: Lineshape = Lineshape.PV):
    ...
    for index, peak in peaks.iterrows():
        str_form = lambda x: "%s%s" % (to_prefix(f"{peak.ASS}_{index}"), x)
        ...
        match lineshape:
            ...
            case lineshape.PV_PV:
                # Use previously fitted values if available
                frac_x = peak.get("fraction_x")
                frac_y = peak.get("fraction_y")
                param_dict[str_form("fraction_x")] = frac_x if not pd.isna(frac_x) else 0.5
                param_dict[str_form("fraction_y")] = frac_y if not pd.isna(frac_y) else 0.5
            case _:
                # Use previously fitted values if available
                frac = peak.get("fraction")
                param_dict[str_form("fraction")] = frac if not pd.isna(frac) else 0.5
...
def make_models(
    ...
    if len(peaks) == 1:
        ...
        mod = Model(model, prefix="%s" % to_prefix(f"{peaks.ASS.iloc[0]}_{peaks.index[0]}"))
        # add parameters
        ...
        mod = Model(model, prefix="%s" % to_prefix(f"{first_peak[1].ASS}_{first_peak[0]}"))
        for index, peak in remaining_peaks:
            mod += Model(model, prefix="%s" % to_prefix(f"{peak.ASS}_{index}"))
...
def update_params(
    ...
    for k, v in param_dict.items():
        ...
        # print("update", k, v)
        if "center" in k:
            if xy_bounds is None or xy_bounds == (0, 0):
                # no bounds set
                pass
            else:
                if "center_x" in k:
...
class FitPeaksArgs:
    ...
    reference_plane_indices: List[int] = field(default_factory=list)
...
def merge_unpacked_parameters_with_metadata(cluster_fit_df, group_of_peaks_df):
    ...
    """
    group_of_peaks_df["prefix"] = [to_prefix(f"{row.ASS}_{idx}") for idx, row in group_of_peaks_df.iterrows()]
    merged_cluster_fit_df = cluster_fit_df.merge(
        group_of_peaks_df, on="prefix", suffixes=["", "_init"]
    )
    # cleanup prefix column
    merged_cluster_fit_df.drop(columns=["prefix"], inplace=True)
